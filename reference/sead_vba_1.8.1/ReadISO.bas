...
fixtureCount = 1
Dim fixtureErrorCount As Integer, fixtureErrorCountCol As Integer, fixtureErrorCountOther As Integer
fixtureErrorCount = 0
fixtureErrorCountCol = 0
fixtureErrorCountOther = 0
ErrorCol = 15
...
For row = 21 To lastRow
    ...
    If TooManyColumns = True Then
        Sheet21.Cells(row, ErrorCol) = Sheet25.Range("tErrorTooMany")
        fixtureErrorCount = fixtureErrorCount + 1
        fixtureErrorCountCol = fixtureErrorCountCol + 1
    ElseIf OtherError = True Then
        Sheet21.Cells(row, ErrorCol) = Sheet25.Range("tErrorUnknown")
        fixtureErrorCount = fixtureErrorCount + 1
        fixtureErrorCountOther = fixtureErrorCountOther + 1
    End If
...
If fixtureErrorCount > 0 Then
    msg = msg & vbNewLine & Sheet25.Range("tUploadErrors") & " "
    msg = msg & fixtureErrorCount & " "
    msg = msg & Sheet25.Range("tOutOf") & " "
    msg = msg & fixtureCount - 1 & " "
    msg = msg & Sheet25.Range("tFixturesWord") & "."
    
    If fixtureErrorCountCol > 0 Then
        msg = msg & vbNewLine & Sheet25.Range("tErrorTooMany")
    End If
    If fixtureErrorCountOther > 0 Then
        msg = msg & vbNewLine & Sheet25.Range("tErrorUnknown")
    End If
    msg = msg & vbNewLine & Sheet25.Range("tErrorLog")
End If
MsgBox (msg)
Sheet13.Protect
End Sub
...
Dim Width, Length, Height As Variant
...
DataRow1Filled = False
DataRow2Filled = False
DataRow1 = 1000000 'Large number as placeholder until all keywords have been identified
DataRow2 = 1000000
VarCount1 = 0
VarCount2 = 0
...
Space = " "
    ...
    If row >= DataRow1 And DataRow1Filled = False Then
        sValue = 0
        Do While Len(sText) > 0
            ...
            If sValue <> "" Then
                ...
                If VarCount1 = 2 Then Lumens = CDbl(sValue)
                If VarCount1 = 3 Then CandelaMult = Val(sValue)
                If VarCount1 = 4 Then
                    NumVertAngles = CInt(sValue)
                End If
                If VarCount1 = 5 Then NumHorizAngles = CInt(sValue)
                If VarCount1 = 6 Then PhotoType = sValue
                If VarCount1 = 7 Then UnitType = sValue
                If VarCount1 = 8 Then Width = sValue        'previously commented
                If VarCount1 = 9 Then Length = sValue       'previously commented
                If VarCount1 = 10 Then
                    Height = sValue
                    DataRow1Filled = True                   'This is the last element in DataRow1
                End If
            End If
        Loop
...
'Assign variables for Data row 2
    If row >= DataRow2 And DataRow1Filled = True And DataRow2Filled = False Then
        sValue = 0
        Do While Len(sText) > 0
            sSpace = InStr(sText, Space)
            If sSpace = 0 Then
            ...
            If sValue <> "" Then
                ...
                If VarCount2 = 3 Then
                    InputWatts = CDbl(sValue)
                    DataRow2Filled = True                   'This is the last element in DataRow2
                End If
            End If
        Loop
    End If
...
'Assign variables for Vertical Angles
    If redimAngles = False And _
        row >= VertAngleRow And _
        DataRow2Filled = True And _
        VertAngleFilled = False And _
        NoKeywords = True _
        Then
            redimAngles = True
    ...
    End If
   If row >= VertAngleRow And DataRow2Filled = True And VertAngleFilled = False And NoKeywords = True Then
        'Set up variables based on the earlier inputs (size of arrays for holding candela data)
...
'Test if FixtureName is used already; if so, add a number to the end (incrementing until a non-unique one is found)
        ...
        increment = 1
        'Force calculation
        Sheet13.EnableCalculation = True
        Range("NumFixturesInLib").Calculate
        Range("FixtureNames").Calculate
        Sheet13.EnableCalculation = False
        testRangeValue = Range("NumFixturesInLib")
'
...
Else    'check to see if we're in compatibility mode or saved as Excel 2007 format
    ...
    If ExcelVersion = "7.0" _
        Or ExcelVersion = "8.0" _
        Or ExcelVersion = "9.0" _
        Or ExcelVersion = "10.0" _
        Or ExcelVersion = "11.0" _
        Then 'Excel 2003 or earlier
